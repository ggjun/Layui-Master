...
/*
 * function openUrl(data){ var dataid = $(data);
 * console.log(dataid.attr("data-url")); parent.openUrl();
 * 
 *  }
 */
...
var getMenus=function(data){
    ...
    for(var i=0;i < data.length;i++){
        ...
        if (node.url && node.url != ""){
        	  a=$("<a class='testa' href='javascript:;' data-url='"+node.url+"'>"+node.name+"</a>");
        }
        else{
